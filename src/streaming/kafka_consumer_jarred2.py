...
Terminal command to run this file from the root project folder:
    ...
    uv run python -m streaming.kafka_consumer_jarred
...
OUTPUT_DB: Final[Path] = OUTPUT_DIR / "sales.duckdb_jarred2"
OUTPUT_CHART: Final[Path] = OUTPUT_DIR / "sales_chart_jarred2.png"
...
def process_message(
    ...
    *,
    ...
    """Process one consumed message safely with exception isolation."""

    # 1. Structural Data Contract Validation
    errors = validate_required_fields(record=row, required_fields=SALES_REQUIRED_FIELDS)
    if errors:
        LOG.warning(f"Validation failed for order {row.get('order_id', '?')}: errors={errors}")
        return None

    # 2. Business Logic & Enrichment (Wrapped safely to prevent fatal crashes)
    try:
        ...
        # Catch unforeseen errors (e.g., KeyError from missing lookup tables, ValueError)
        LOG.error(f"!!! CRITICAL PROCESSING ERROR !!! Failed to enrich order {row.get('order_id', '?')}")
        LOG.error(f"Error details: {type(err).__name__}: {str(err)}")

        # Write the raw broken payload to a local 'dead letter' file for later engineering analysis
        dlq_path = OUTPUT_DIR / "dead_letter_payloads.log"
        with open(dlq_path, "a") as dlq_file:
            dlq_file.write(f"ERROR: {type(err).__name__} | PAYLOAD: {row}\n")
...
def consume_messages(
    ...
    *,
    ...
    while consumed_count + skipped_count < MAX_MESSAGES:
        ...
        if enriched is None:
            skipped_count += 1
            total_processed = consumed_count + skipped_count
            failure_rate = (skipped_count / total_processed) * 100

            LOG.warning("!!! MESSAGE REJECTED VIA DATA CONTRACT !!!")
            LOG.warning(f"order={row.get('order_id', '?')}")
            LOG.warning(f"Pipeline Data Health: {failure_rate:.1f}% messages failing contract validation")
            continue
...
def log_summary(
    ...
    """Log final summary statistics including data quality metrics."""
    LOG.info("==================================================")
    LOG.info("FINAL STREAM SUMMARY")
    LOG.info("==================================================")
    LOG.info(f"Total Attempted Messages: {consumed_count + skipped_count}")
    LOG.info(f"  - Successfully Processed: {consumed_count}")
    LOG.info(f"  - Rejected (Contract Breaches): {skipped_count}")

    # Calculate and log validation failure rate
    total_messages = consumed_count + skipped_count
    if total_messages > 0:
        fail_rate = (skipped_count / total_messages) * 100
        LOG.info(f"  - Data Contract Failure Rate: {fail_rate:.2f}%")

    log_path(LOG, "OUTPUT_CSV", OUTPUT_CSV)

    if stats.count > 0:
        LOG.info(f"  Total sales value:  ${stats.total:,.2f}")
        LOG.info(f"  Average sale value: ${stats.mean:,.2f}")

    LOG.info("========================")
    LOG.info("Consumer pipeline execution finalized successfully.")
    LOG.info("========================")
